...
export async function PUT(req: NextRequest) {
  try {
    ...
    if (isFollowing) {
      // Unfollow: Remove current user's ID from target's followers
      await User.findByIdAndUpdate(
        UserToFollow._id,
        { $pull: { followers: currentUser._id } }
      );
      // Unfollow: Remove target user's ID from current user's following list
      await User.findByIdAndUpdate(
        currentUser._id, // Use the MongoDB _id
        { $pull: { following: UserToFollow._id } }
      );
    } else {
      // Follow: Add current user's ID to target's followers
      await User.findByIdAndUpdate(
        UserToFollow._id,
        { $push: { followers: currentUser._id } }
      );
      // Follow: Add target user's ID to current user's following list
      await User.findByIdAndUpdate(
        currentUser._id, // Use the MongoDB _id
    ...
    return NextResponse.json({ msg: "Success" }, { status: 200 }); // Changed status to 200 for a successful update
  } catch (err) {
